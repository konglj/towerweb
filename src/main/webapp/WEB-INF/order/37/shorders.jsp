							...
							<div class="row">
								<div class="col-md-6" style="padding-right:0px;">
									<select name="parameter.towercity" id="towercity" class="form-control" ref="towerdistrict">
										 <c:forEach items="${orderPage.paramerCitys }" var="city">
      							  		 <option value="${city.cityid}" <c:if test='${city.cityid==orderPage.parameter.towercity }'>selected="selected"</c:if>  >${city.cityname }</option>
      								    </c:forEach>
      								</select>
								</div>
								<div class="col-md-6  style="padding-right:0px;" >
					...
					<div class="form-group">
						...
						<div class="col-md-8">
							...
							<select name="parameter.usercity" id="usercity" class="form-control" ref="userdistrict">
										 <c:forEach items="${orderPage.userParamerCitys }" var="city">
      							  		 <option value="${city.cityid}" <c:if test='${city.cityid==orderPage.parameter.usercity }'>selected="selected"</c:if>  >${city.cityname }</option>
      								    </c:forEach>
      								</select>
      						</div>		
      					<div class="col-md-6" style="padding-right:0px;">
...
<script src="<%=basePath%>assets/js/page/order/37/shorders.js"></script>
