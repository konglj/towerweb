	...
	<div class="portlet-body">
		...
		<form role="form" class="margin-bottom-10 margin-top-10">
			<div class="row">
						...
						<div class="col-md-9">
							<div class="row">
								<div class="col-md-6  style="padding-right:0px;" >
									<select id="city" name="city" class="form-control combox" ref="district">
									 <c:forEach items="${towersPage.paramerCitys }" var="city">
      							  		<option value="${city.cityid}" <c:if test='${city.cityid==towersPage.city }'>selected="selected"</c:if>  >${city.cityname }</option>
      								 </c:forEach>
      								</select>
								</div>
								<div class="col-md-6" style="padding-right:0px;">
				...
				<div class="col-md-2">
					<div class="form-group">
						<label class="control-label col-md-6"> 站址状态</label>
						<div class="col-md-6">
							<select name="towerstate" id="towerstate" class="form-control combox" >
      							<c:forEach items="${towersPage.towerStates }" var="state">
      							  <option value="${state.state }" <c:if test='${state.state==towersPage.towerstate }'>selected="selected"</c:if>  >${state.name }</option>
      							</c:forEach>
							</select>
						</div>
					</div>
		...
		<div class="portlet-body table-responsive1 " >
			...
			<div class="btn-group">
				<a href="javascript:;" onclick="check_auth(0,'A2',updateTowerVisible);">
					...
					选中站址下架<i class="icon-arrow-down"></i>
				</button>
				</a>
...
<script src="<%=basePath%>assets/js/page/tower/37/tower_yes.js"></script>
