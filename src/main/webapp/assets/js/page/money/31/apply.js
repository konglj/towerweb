		...
		init: function() {
			$('#sample_1')
				.dataTable({
					...
					"sAjaxSource": $.path +"/money/get_txs.html?apply=1", // 方法名 selectAll
					"columns": [
					     /*
					      {
					...
					columnDefs: [
						
						{
							...
							render: function(a, b, c, d) {
								...
								var id = c.id;
								var isdeal=c.isdeal;
								var res='<a   onclick=check_auth_arg('+id+',0,"A1",GetApplyInfo)><span class="glyphicon glyphicon-search"  title="查看" ></span> </a>';
								if(isdeal==1){
									res+='<a   onclick=check_auth_arg('+id+',0,"A1",ShApplyInfo)><span class="glyphicon glyphicon-cog"  title="审核" ></span> </a>';
								}
								return res;
